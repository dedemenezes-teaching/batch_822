...
describe 'OrdersController', :_order do
  ...
  let(:order_repository) do
    OrderRepository.new(orders_csv_path, meal_repository, customer_repository, employee_repository)
  end
  ...
  describe '#mark_as_delivered' do
    ...
    it 'should ask the rider for an order index (of their undelivered orders), mark it as delivered, and save the relevant data to the CSV file' do
      ...
      # Reload from CSV
      new_order_repository = OrderRepository.new(orders_csv_path, meal_repository, customer_repository,
                                                 employee_repository)
